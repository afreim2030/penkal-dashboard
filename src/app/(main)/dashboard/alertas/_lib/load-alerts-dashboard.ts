...
export async function loadAlertsDashboard(): Promise<AlertsDashboardData> {
  ...
  const [products, inbounds, conflictsResult, campaignsResult] = await Promise.all([
    loadProductsDashboard(), loadFullInboundsDashboard(),
  ...
  const affected = (products?.products ?? []).filter((product) => product.stockTimeAffected > 0).sort((left, right) => right.stockTimeAffected - left.stockTimeAffected).slice(0, 20);
  for (const product of affected) alerts.push({ id: `stock-time-${product.sku}`, severity: "warning", category: "Estoque FULL", title: `${product.sku} afetando tempo de estoque`, description: `${product.name} · ${product.fullStock} un. no FULL`, value: `${product.stockTimeAffected} un. afetadas`, href: "/dashboard/estoque-full" });
